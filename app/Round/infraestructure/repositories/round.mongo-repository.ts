...
export class RoundMongoRepository implements RoundRepository {
  ...
  public closeBetsInRound = async (uuid: string): Promise<RoundEntity | null> => {
    try {
      const roundBetsClosed = await RoundModel.findOneAndUpdate(
        { uuid },
        { open: false },
        { new: true },
      )
  ...
  public setJackpotInRound = async (
    uuid: string,
    jackpotValue: Jackpot,
  ): Promise<RoundEntity | null> => {
    try {
      const roundWithJackpot = await RoundModel.findOneAndUpdate(
        { uuid },
  ...
  public closeRound = async (uuid: string, result: number): Promise<RoundEntity | null> => {
    try {
      const roundClosed = await RoundModel.findOneAndUpdate(
        { uuid },
        { result, providerId: '999' },
        { new: true },
      )
  ...
  public closeRounds = async (providerId: string, result: number): Promise<any> => {
    try {
      const roundsClosed = await RoundModel.updateMany(
        { providerId },
        { result, providerId: '999' },
      )
      return roundsClosed
