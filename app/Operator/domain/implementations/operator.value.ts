...
export class Operator implements OperatorEntity {
  ...
  getGameInOperator(gameUuid: string): Boolean {
    const gameFound = this.operatorGamesLimits?.find(gameLimit => ( gameLimit.game.uuid === gameUuid ));
    return Boolean(gameFound)
  }

  addOperatorGamesLimits(operatorGamesLimits: OperatorGameLimitsEntity) {
    // Busca si este operador ya tiene ese juego
    const gameLimit = this.operatorGamesLimits?.find(gameLimit => (
      gameLimit.game.uuid === operatorGamesLimits.game.uuid
    ));
    ...
    const gameLimitFound = this.operatorGamesLimits?.findIndex(gameLimit => (
      gameLimit.game.uuid === operatorGamesLimits.game.uuid
    ));

    // Valida si ya hay una configuracion con la currency entrante para este juego
    const currencyFound = gameLimit.currencyAndLimits.findIndex( currencyLimit => (currencyLimit.currency.uuid === operatorGamesLimits.currencyAndLimits[0].currency.uuid));
    if(currencyFound !== -1 ) {
    ...
    this.operatorGamesLimits![gameLimitFound!].currencyAndLimits.push(...operatorGamesLimits.currencyAndLimits);
    return this.operatorGamesLimits;
  };

  addConfigPaymentByGame(gameUuid: string, configPayment: ConfigPaymentEntity[]) {
    const gameFound = this.getGameInOperator(gameUuid);
    if(!gameFound) return null;

    const gameIdx = this.operatorGamesLimits?.findIndex(gameLimit => (gameLimit.game.uuid === gameUuid));
    console.log({gameIdx})
