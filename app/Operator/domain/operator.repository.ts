...
export interface OperatorRepository {
  ...
  getGameAndLimitsInOperator(params: {
    uuid?: string
  ...
  // CONFIG PAYMENTS
  addConfigPaymentInGame(
    uuid: string,
    gameUuid: string,
    configPayment: ConfigPaymentEntity[],
  ): Promise<OperatorEntity | null>
  // INTEGRATION LOBBY
  getGamesByCurrencyInOperator(
    uuid: string,
    isoCode: string,
  ): Promise<OperatorGameLimitsEntity[] | []>
  getGames(uuid: string): Promise<OperatorGameLimitsEntity[] | []>
}
