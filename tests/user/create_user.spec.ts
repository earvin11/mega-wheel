...
test.group('User: Create user.', (group) => {
  ...
  test('Token not found: Should return a 400 status code and an error message', async ({
    ...
    const response = await client.post('/user/create').header('authentication-x-token', 'asd')
  ...
  test('Client do not exist: Should return a 404 status code and an error message', async ({
    client,
  }) => {
    const user = await client.post('/auth/login').json({ userName: 'user', password: 'user' })
    const response = await client
  ...
  test('Operator do not exist: Should return a 404 status code and an error message', async ({
    client,
  }) => {
    const user = await client.post('/auth/login').json({ userName: 'user', password: 'user' })
    const response = await client
